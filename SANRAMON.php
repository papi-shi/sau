...
function getTotalWorth($substation = 'sanramon') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'sanramon'));
...
function getMonthlyReportData($month, $year, $substation = 'sanramon') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'sanramon') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'sanramon') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'sanramon'; // Only show reports for sanramon substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from sanramon substation
    $where = ["substation = 'sanramon'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from sanramon substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'sanramon' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for sanramon substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'sanramon' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station sanramon</h3>
    ...
    <nav>
        <ul>
            <li><a href="sanramon.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="sanramon.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="sanramon.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="sanramon.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="sanramon.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="sanramon" selected>sanramon</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="sanramon.php" method="get">
