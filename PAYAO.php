...
function getTotalWorth($substation = 'PAYAO') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'PAYAO'));
...
function getMonthlyReportData($month, $year, $substation = 'PAYAO') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'PAYAO') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'PAYAO') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'PAYAO',  -- NEW FIELD ADDED
...
$substation = 'PAYAO'; // Only show reports for PAYAO substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from PAYAO substation
    $where = ["substation = 'PAYAO'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from PAYAO substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'PAYAO' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for PAYAO substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'PAYAO' GROUP BY status";
...
<body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station PAYAO</h3>
    ...
    <nav>
        <ul>
            <li><a href="PAYAO.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="PAYAO.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="PAYAO.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="PAYAO.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="PAYAO.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="PAYAO" selected>PAYAO</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="PAYAO.php" method="get">
