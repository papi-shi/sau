...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'ARENA BLANCO'));
    ...
    try {
        $query = "INSERT INTO incidents (
            incident_type, description, substation, status, person_status, vessels_status, personnel_name, location, latitude, 
        ...
        $bind_result = $stmt->bind_param(
            ...
            $incident_type, $description, $substation, $status, $person_status, $vessels_status, $personnel_name, $location, $lat,
...
function getMonthlyReportData($month, $year, $substation = 'ARENA BLANCO') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'ARENA BLANCO') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'ARENA BLANCO') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'ARENA BLANCO',  -- NEW FIELD ADDED
...
$substation = 'ARENA BLANCO'; // Only show reports for ARENA BLANCO substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from ARENA BLANCO substation
    $where = ["substation = 'ARENA BLANCO'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from ARENA BLANCO substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'ARENA BLANCO' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for ARENA BLANCO substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'ARENA BLANCO' GROUP BY status";
...
<body>
    ...
    <nav>
        <ul>
            <li><a href="ARENA BLANCO.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="ARENA BLANCO.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="ARENA BLANCO.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="ARENA BLANCO.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="ARENA BLANCO.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="ARENA BLANCO" selected>ARENA BLANCO</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="ARENA BLANCO.php" method="get">
