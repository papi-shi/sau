...
function getTotalWorth($substation = 'naga') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'naga'));
...
function getMonthlyReportData($month, $year, $substation = 'naga') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'naga') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'naga') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'naga'; // Only show reports for naga substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from naga substation
    $where = ["substation = 'naga'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from naga substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'naga' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for naga substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'naga' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station naga</h3>
    ...
    <nav>
        <ul>
            <li><a href="naga.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="naga.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="naga.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="naga.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="naga.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="naga" selected>naga</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="naga.php" method="get">
