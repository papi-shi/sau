...
function getTotalWorth($substation = 'TALUSAN') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'TALUSAN'));
...
function getMonthlyReportData($month, $year, $substation = 'TALUSAN') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'TALUSAN') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'TALUSAN') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'TALUSAN',  -- NEW FIELD ADDED
...
$substation = 'TALUSAN'; // Only show reports for TALUSAN substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from TALUSAN substation
    $where = ["substation = 'TALUSAN'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from TALUSAN substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'TALUSAN' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for TALUSAN substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'TALUSAN' GROUP BY status";
...
<body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station TALUSAN</h3>
    ...
    <nav>
        <ul>
            <li><a href="TALUSAN.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="TALUSAN.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="TALUSAN.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="TALUSAN.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="TALUSAN.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="TALUSAN" selected>TALUSAN</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="TALUSAN.php" method="get">
