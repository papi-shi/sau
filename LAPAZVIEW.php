...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'LAPAZ'));
...
function getMonthlyReportData($month, $year, $substation = 'LAPAZ') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'LAPAZ') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'LAPAZ') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'LAPAZ',  -- NEW FIELD ADDED
...
$substation = 'LAPAZ'; // Only show reports for LAPAZ substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from LAPAZ substation
    $where = ["substation = 'LAPAZ'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from LAPAZ substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'LAPAZ' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for LAPAZ substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'LAPAZ' GROUP BY status";
...
<body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station LAPAZ</h3>
    </div>
    
    <nav>
        <ul>
            <li><a href="LAPAZview.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="LAPAZview.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="LAPAZview.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="LAPAZview.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="LAPAZview.php" method="get">
