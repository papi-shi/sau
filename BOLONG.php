...
function getTotalWorth($substation = 'bolong') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'bolong'));
    ...
    // NEW: Religion and Ethnicity fields
    $religion = htmlspecialchars(trim($_POST['religion'] ?? ''), ENT_QUOTES);
    $ethnicity = htmlspecialchars(trim($_POST['ethnicity'] ?? ''), ENT_QUOTES);
    ...
    // Insert to DB with new religion and ethnicity fields
try {
        $query = "INSERT INTO incidents (
            ...
            religion, ethnicity, date_time, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())";
        ...
        $bind_result = $stmt->bind_param(
            "sssssssdssddsssssssssssddssissssssss", 
...
function getMonthlyReportData($month, $year, $substation = 'bolong') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'bolong') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'bolong') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'bolong'; // Only show reports for bolong substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from bolong substation
    $where = ["substation = 'bolong'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from bolong substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'bolong' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for bolong substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'bolong' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station bolong</h3>
    </div>
    
    <nav>
        <ul>
            <li><a href="bolong.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="bolong.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="bolong.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="bolong.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="bolong.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
    
    <div class="container">
        <!-- ... your PHP and HTML content remains unchanged ... -->
    ...
    <style>
        ...
        /* Make "Add New Incident Report" text black */
        #incident-modal .modal-content h2 {
            color: #000 !important;
        }
        ...
        .card, .modal-content {
            ...
            padding: 1.5rem;
            margin-bottom: 2rem;
            color: #000 !important; /* Ensure all text is visible (black) */
        }
        .grid {
            display: flex;
            flex-wrap: wrap;
    ...
    <!-- ... rest of your JavaScript and HTML ... -->
     <div class="container">
        <?php if (!empty($errors)): ?>
        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($incident['longitude']); ?></td>
                    <td><?php echo htmlspecialchars($incident['latitude']); ?></td>
                    <td><?php echo htmlspecialchars($incident['personnel_name']); ?></td>
                    <td>
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-secondary btn-sm" onclick="showFullIncidentModal(<?php echo $incident['id']; ?>)">
                                <i class="fas fa-list"></i> Full Data
                            </button>
                        </div>

                        <!-- Full Data Modal (one per row, hidden by default) -->
                        <div id="full-incident-modal-<?php echo $incident['id']; ?>" class="modal" style="display:none;">
                            <div class="modal-content" style="max-width:900px;">
                                <span class="modal-close" onclick="closeFullIncidentModal(<?php echo $incident['id']; ?>)">&times;</span>
                                <h3>Incident Data (ID: <?php echo $incident['id']; ?>)</h3>
                                <div style="max-height:70vh;overflow:auto;">
                                    <table class="table table-bordered table-striped">
                                        <tbody>
                                            <?php
                                            // Define fields to show per incident type
                                            $type = strtoupper($incident['INCIDENT_TYPE']);
                                            $common_fields = [
                                                'INCIDENT_TYPE' => 'Incident Type',
                                                'description' => 'Description',
                                                'date_time' => 'Date & Time',
                                                ...
                                                'latitude' => 'Latitude',
                                                'longitude' => 'Longitude',
                                                'personnel_name' => 'Reporting Personnel',
                                                'status' => 'Case Status',
                                                'person_status' => 'Person Status',
                                                'vessels_status' => 'Vessel Status',
                                                'report' => 'Detailed Report',
                                                'personnel_image' => 'Personnel Image',
                                                'incident_image' => 'Incident Image',
                                                'religion' => 'Religion',
                                                'ethnicity' => 'Ethnicity',
                                            ];
                                            $type_fields = [
                                                'IUUF' => array_merge($common_fields, [
                                                    'worth' => 'Total Worth of Confiscated Items',
                                                ]),
                                                'ILLEGAL DRUGS' => array_merge($common_fields, [
                                                    'quantity' => 'Quantity',
                                                    'value' => 'Value',
                                                    'weight' => 'Weight',
                                                ]),
                                                'SMUGGLING' => array_merge($common_fields, [
                                                    'goods_vessel_type' => 'Type of Goods/Vessel',
                                                    'other_vessel_type' => 'Other Vessel Type',
                                                    'port_origin' => 'Port of Origin',
                                                    'port_destination' => 'Port of Destination',
                                                    'flag_registry' => 'Flag of Registry',
                                                    'flag_convenience' => 'Flag of Convenience',
                                                ]),
                                                'HUMAN TRAFFICKING' => array_merge($common_fields, [
                                                    'name' => 'Name',
                                                    'age' => 'Age',
                                                    ...
                                                    'vessel_type_ht' => 'Vessel Type',
                                                    'other_vessel_type_ht' => 'Other Vessel Type',
                                                ]),
                                                // Add more types as needed, fallback to common_fields
                                            ];
                                            $fields_to_show = $type_fields[$type] ?? $common_fields;
                                            foreach ($fields_to_show as $key => $label):
                                                if (!array_key_exists($key, $incident)) continue;
                                                $value = $incident[$key];
                                            ?>
                                                <tr>
                                                    <th style="text-transform:capitalize;"><?php echo htmlspecialchars($label); ?></th>
                                                    <td>
                                                        <?php
                                                        if (in_array($key, ['personnel_image', 'incident_image']) && !empty($value)) {
                                                            echo '<img src="' . htmlspecialchars($value) . '" style="max-width:120px;max-height:120px;border-radius:6px;">';
                                ...
                                <button class="btn btn-primary" onclick="printIncidentFullData(<?php echo $incident['id']; ?>)">
                                    <i class="fas fa-print"></i> Print
                                </button>
                                <button class="btn btn-secondary" onclick="closeFullIncidentModal(<?php echo $incident['id']; ?>)">
                                    Close
                                </button>
                            </div>
                        </div>
                        <script>
                        function showFullIncidentModal(id) {
                            document.getElementById('full-incident-modal-' + id).style.display = 'block';
                        }
                        function closeFullIncidentModal(id) {
                            document.getElementById('full-incident-modal-' + id).style.display = 'none';
                        }
                        function printIncidentFullData(id) {
                            var modal = document.getElementById('full-incident-modal-' + id);
                            var content = modal.querySelector('.modal-content').innerHTML;
                            var printWindow = window.open('', '', 'width=900,height=700');
                            printWindow.document.write(`
                                <html>
                                <head>
                                    <title>Print Incident Full Data</title>
                                    <style>
                                        body { font-family: Arial, sans-serif; padding: 20px; }
                                        table { width: 100%; border-collapse: collapse; }
                                        th, td { border: 1px solid #ccc; padding: 8px; text-align: left; }
                                        th { background-color: #f2f2f2; }
                                        img { max-width: 120px; max-height: 120px; border-radius: 6px; }
                                    </style>
                                </head>
                                <body>
                                    <h2>Incident Full Data</h2>
                                    ${content}
                                </body>
                                </html>
                            `);
        ...
        <form method="post" enctype="multipart/form-data" id="incident-form">
            ...
            <div class="grid">
                <div>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="bolong" selected>bolong</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group" id="other_ethnicity_group" style="display: none;">
                        <label for="other_ethnicity">Specify Ethnicity</label>
                        <input type="text" name="other_ethnicity" id="other_ethnicity">
                    </div>
                    <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        const ethnicitySelect = document.getElementById('ethnicity');
                        const otherEthnicityGroup = document.getElementById('other_ethnicity_group');
                        const otherEthnicityInput = document.getElementById('other_ethnicity');
                        ethnicitySelect.addEventListener('change', function() {
                            if (this.value === 'OTHERS') {
                                otherEthnicityGroup.style.display = 'block';
                                otherEthnicityInput.setAttribute('required', 'required');
                            } else {
                                otherEthnicityGroup.style.display = 'none';
                                otherEthnicityInput.removeAttribute('required');
                                otherEthnicityInput.value = '';
                            }
                        });
                    });
                    </script>
                    <div class="form-group" id="other_religion_group" style="display: none;">
                        <label for="other_religion">Specify Religion</label>
                        <input type="text" name="other_religion" id="other_religion">
                    </div>
                    <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        const religionSelect = document.getElementById('religion');
                        const otherReligionGroup = document.getElementById('other_religion_group');
                        const otherReligionInput = document.getElementById('other_religion');
                        religionSelect.addEventListener('change', function() {
                            if (this.value === 'OTHERS') {
                                otherReligionGroup.style.display = 'block';
                                otherReligionInput.setAttribute('required', 'required');
                            } else {
                                otherReligionGroup.style.display = 'none';
                ...
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <div style="margin-top:8px;">
                            ...
                            <input type="text" name="latitude" id="latitude" required style="width: 120px; display: inline-block;" placeholder="Latitude">
                            &nbsp;
                            <strong>Longitude:</strong>
                            <input type="text" name="longitude" id="longitude" required style="width: 120px; display: inline-block;" placeholder="Longitude">
                        </div>
                    </div>
                </div>
                <script>
                document.addEventListener('DOMContentLoaded', function() {
                    // Update lat/lng display and marker when input fields are changed manually
                    ...
                    // Search location and zoom map
                    const searchBtn = document.getElementById('search-location-btn');
                    const locationInput = document.getElementById('location');
                    searchBtn.addEventListener('click', function() {
                        const query = locationInput.value.trim();
                        if (!query) {
                            alert('Please enter an address, barangay, or municipality to search.');
                            return;
                        }
                        ...
                        fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}`)
                            ...
                            .then(data => {
                                if (data && data.length > 0) {
                                    ...
                                    alert('Location not found. Please try a different address or spelling.');
                                }
                            })
                            .catch(() => {
                    ...
                    // Also update lat/lng display on page load if values exist
                    updateLatLngDisplay();
                });
                </script>
            </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="bolong.php" method="get">
