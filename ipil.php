...
function getTotalWorth($substation = 'ipil') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'ipil'));
...
function getMonthlyReportData($month, $year, $substation = 'ipil') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'ipil') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'ipil') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'ipil'; // Only show reports for ipil substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from ipil substation
    $where = ["substation = 'ipil'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from ipil substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'ipil' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for ipil substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'ipil' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station ipil</h3>
    ...
    <nav>
        <ul>
            <li><a href="ipil.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="ipil.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="ipil.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="ipil.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="ipil.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="ipil" selected>ipil</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="ipil.php" method="get">
