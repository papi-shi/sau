...
function getTotalWorth($substation = 'sangali') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'sangali'));
...
function getMonthlyReportData($month, $year, $substation = 'sangali') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'sangali') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'sangali') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'sangali'; // Only show reports for sangali substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from sangali substation
    $where = ["substation = 'sangali'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from sangali substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'sangali' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for sangali substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'sangali' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station sangali</h3>
    ...
    <nav>
        <ul>
            <li><a href="sangali.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="sangali.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="sangali.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="sangali.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="sangali.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="sangali" selected>sangali</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="sangali.php" method="get">
