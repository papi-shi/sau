...
function getTotalWorth($substation = 'riohondo') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'riohondo'));
...
function getMonthlyReportData($month, $year, $substation = 'riohondo') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'riohondo') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'riohondo') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'riohondo'; // Only show reports for riohondo substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from riohondo substation
    $where = ["substation = 'riohondo'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from riohondo substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'riohondo' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for riohondo substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'riohondo' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station riohondo</h3>
    ...
    <nav>
        <ul>
            <li><a href="riohondo.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="riohondo.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="riohondo.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="riohondo.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="riohondo.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="riohondo" selected>riohondo</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="riohondo.php" method="get">
