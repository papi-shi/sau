...
$query = "SELECT age FROM incidents WHERE INCIDENT_TYPE = 'HUMAN TRAFFICKING' AND age IS NOT NULL";
...
<head>
    ...
    <title>Human Trafficking Incidents - Age Statistics</title>
...
<body>
    <div class="page-wrapper">
        <header class="dashboard-header">
            <h1 class="dashboard-title">Human Trafficking Incidents - Age Distribution</h1>
        </header>
        ...
        <section class="data-panel">
            <table class="data-table">
                ...
                <tbody>
                    <?php
                    $totalIncidents = array_sum($ageStats);
                    foreach ($ageStats as $group => $count) {
                        $percentage = $totalIncidents > 0 ? round(($count / $totalIncidents) * 100, 2) : 0;
                        echo "<tr>
                                <td>{$group} years</td>
                                <td>{$count}</td>
                                <td>{$percentage}%</td>
                    ...
                    <tr class="total-row">
                        ...
                        <td><strong><?php echo $totalIncidents; ?></strong></td>
        ...
        <div class="navigation-panel">
            <a href="incidents.php" class="back-button"><i class="fas fa-arrow-left"></i> Back to Incidents</a>
        </div>
    </div>
                    ...
                    chart.data.datasets.forEach((dataset, datasetIndex) => {
                        ...
                        if (!meta.hidden) {
                            meta.data.forEach((element, index) => {
                                ...
                                ctx.fillText(data, position.x, position.y - 20);
                            });
                        }
                    });
                }
...
<style>
    /* trafficking_statistics.css - Custom styling for Human Trafficking Age Statistics */
...
<body>
    ...
    <nav>
        <ul>
            <li><a href="humantrafficking.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="humantrafficking.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="humantrafficking.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="humantrafficking.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="humantrafficking.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
...
<div class="table-responsive">
    ...
    <table class="table table-striped table-bordered">
        ...
        <tbody>
            ...
            // Query for human trafficking incidents only
$query = "SELECT * FROM incidents WHERE INCIDENT_TYPE = 'HUMAN TRAFFICKING' ORDER BY date_time DESC";
$incidents = $conn->query($query);
            ...
            ?>
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($incident['name'] ?? 'N/A'); ?></td>
    <td><?php echo htmlspecialchars($incident['age'] ?? 'N/A'); ?></td>
    <td><?php echo !empty($incident['birth_date']) ? date('M d, Y', strtotime($incident['birth_date'])) : 'N/A'; ?></td>
    <td><?php echo htmlspecialchars($incident['sex'] ?? 'N/A'); ?></td>
    <td><?php echo htmlspecialchars($incident['civil_status'] ?? 'N/A'); ?></td>
    <td><?php echo htmlspecialchars($incident['citizenship'] ?? 'N/A'); ?></td>
    <td><?php echo htmlspecialchars($incident['occupation'] ?? 'N/A'); ?></td>
  ...
  data-name="<?php echo htmlspecialchars($incident['name'] ?? 'N/A'); ?>"
  data-age="<?php echo htmlspecialchars($incident['age'] ?? 'N/A'); ?>"
  data-birth="<?php echo !empty($incident['birth_date']) ? date('M d, Y', strtotime($incident['birth_date'])) : 'N/A'; ?>"
  data-sex="<?php echo htmlspecialchars($incident['sex'] ?? 'N/A'); ?>"
  data-civil="<?php echo htmlspecialchars($incident['civil_status'] ?? 'N/A'); ?>"
  data-citizen="<?php echo htmlspecialchars($incident['citizenship'] ?? 'N/A'); ?>"
  data-occupation="<?php echo htmlspecialchars($incident['occupation'] ?? 'N/A'); ?>"
  data-rescued="<?php echo htmlspecialchars($incident['rescued'] ?? ''); ?>"
  ...
  <div class="modal-content">
    ...
    <div class="profile-body">
      ...
      <di class="profile-details">
        ...
        <p><strong>Birth Date:</strong> <span id="profileBirth"></span></p>
        <p><strong>Sex:</strong> <span id="profileSex"></span></p>
        <p><strong>Civil Status:</strong> <span id="profileCivil"></span></p>
        <p><strong>Citizenship:</strong> <span id="profileCitizen"></span></p>
        <p><strong>Occupation:</strong> <span id="profileOccupation"></span></p>
        <p><strong>Rescued:</strong> <span id="profileRescued"></span></p>
...
document.querySelectorAll('.view-details').forEach(button => {
    button.addEventListener('click', function() {
        ...
        document.getElementById('profileName').innerText = this.getAttribute('data-name') || 'N/A';
        document.getElementById('profileAge').innerText = this.getAttribute('data-age') || 'N/A';
        document.getElementById('profileBirth').innerText = this.getAttribute('data-birth') || 'N/A';
        document.getElementById('profileSex').innerText = this.getAttribute('data-sex') || 'N/A';
        document.getElementById('profileCivil').innerText = this.getAttribute('data-civil') || 'N/A';
        document.getElementById('profileCitizen').innerText = this.getAttribute('data-citizen') || 'N/A';
        document.getElementById('profileOccupation').innerText = this.getAttribute('data-occupation') || 'N/A';
        document.getElementById('profileRescued').innerText = this.getAttribute('data-rescued') || '';
            ...
            <div class="grid">
                <div>
                    ...
                    <div id="human_trafficking_fields" style="display: none;">
                        ...
                        <!-- Added missing vessel type for human trafficking -->
                        <div class="form-group">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const incidentDescriptions = {
            ...
            "ITOFP": ["WITHOUT THE REQUISITE AUTHORIZATION", "WITH INCOMPLETE REQUIRED SUPPORTING DOCUMENTS", "WITH GENUINE AUTHORIZATIONS OR PERMITS AND/OR SUPPORTING DOCCUMENTATION THAT HAVE AN EXPIRED VALIDITY", "WITH SPURIOUS (FAKE) AUTHORIZATION'S, PERMITS AND/OR SUPPORTING DOCUMENTS"],
        ...
        incidentTypeSelect.addEventListener('change', function() {
            ...
            } else if (this.value === 'HUMAN TRAFFICKING') {
...
<div id="incident-modal" class="modal">
  <div class="modal-content profiling-layout"> <!-- added 'profiling-layout' -->
    <span class="modal-close">&times;</span>
    <h2>Add New Incident Report</h2>
    ...
    <form method="post" enctype="multipart/form-data" id="incident-form">
      ...
      <div class="profiling-container">
        <div class="incident-image-side">
          <h3>Incident Image</h3>
          <div class="form-group">
            <input type="file" name="incident_image" id="incident_image" accept="image/*" required>
            <div id="incident-preview" class="incident-preview"></div> <!-- added a class -->
          </div>
        </div>

        <l class="incident-details-side">
          <div class="grid">
            <div>
              <!-- Incident Type, Description, Date Time, Reporting Personnel -->
              <div class="form-group">
                <label for="incident_type">Incident Type</label>
                <select name="INCIDENT_TYPE" id="incident_type" required>
                  <option value="">Select Type</option>
              ...
              <div class="form-group">
                <h3>Personnel Image (Optional)</h3>
                <div class="camera-box">
                  <video id="video" autoplay style="display: block;"></video>
                  <img id="preview" class="preview-img" style="display: none;">
                  <input type="hidden" name="camera_image_data" id="camera_image_data">
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="humantrafficking.php" method="get">
