...
function getTotalWorth($substation = 'OLUTANGA') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'OLUTANGA'));
...
function getMonthlyReportData($month, $year, $substation = 'OLUTANGA') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'OLUTANGA') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'OLUTANGA') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'OLUTANGA',  -- NEW FIELD ADDED
...
$substation = 'OLUTANGA'; // Only show reports for OLUTANGA substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from OLUTANGA substation
    $where = ["substation = 'OLUTANGA'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from OLUTANGA substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'OLUTANGA' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for OLUTANGA substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'OLUTANGA' GROUP BY status";
...
<body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station OLUTANGA</h3>
    ...
    <nav>
        <ul>
            <li><a href="OLUTANGA.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="OLUTANGA.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="OLUTANGA.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="OLUTANGA.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="OLUTANGA.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="OLUTANGA" selected>OLUTANGA</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="OLUTANGA.php" method="get">
