...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'SAN RAMON'));
...
function getMonthlyReportData($month, $year, $substation = 'SAN RAMON') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'SAN RAMON') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'SAN RAMON') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'SAN RAMON',  -- NEW FIELD ADDED
...
$substation = 'SAN RAMON'; // Only show reports for SAN RAMON substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from SAN RAMON substation
    $where = ["substation = 'SAN RAMON'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from SAN RAMON substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'SAN RAMON' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for SAN RAMON substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'SAN RAMON' GROUP BY status";
...
<body>
    ...
    <nav>
        <ul>
            <li><a href="SAN_RAMON.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="SAN_RAMON.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="SAN_RAMON.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="SAN_RAMON.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="SAN_RAMON.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="SAN RAMON" selected>SAN RAMON</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="SAN RAMON.php" method="get">
