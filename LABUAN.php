...
function getTotalWorth($substation = 'labuan') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'labuan'));
...
function getMonthlyReportData($month, $year, $substation = 'labuan') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'labuan') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'labuan') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'labuan'; // Only show reports for labuan substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from labuan substation
    $where = ["substation = 'labuan'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from labuan substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'labuan' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for labuan substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'labuan' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station labuan</h3>
    ...
    <nav>
        <ul>
            <li><a href="labuan.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="labuan.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="labuan.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="labuan.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="labuan.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="labuan.php" method="get">
