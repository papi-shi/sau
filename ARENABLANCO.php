...
function getTotalWorth($substation = 'arenablanco') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'arenablanco'));
...
function getMonthlyReportData($month, $year, $substation = 'arenablanco') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'arenablanco') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'arenablanco') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'arenablanco'; // Only show reports for arenablanco substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from arenablanco substation
    $where = ["substation = 'arenablanco'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from arenablanco substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'arenablanco' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for arenablanco substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'arenablanco' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station arenablanco</h3>
    ...
    <nav>
        <ul>
            <li><a href="arenablanco.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="arenablanco.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="arenablanco.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="arenablanco.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="arenablanco.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="arenablanco" selected>arenablanco</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="arenablanco.php" method="get">
