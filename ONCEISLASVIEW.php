...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'ONCEISLAS'));
...
function getMonthlyReportData($month, $year, $substation = 'ONCEISLAS') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'ONCEISLAS') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'ONCEISLAS') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'ONCEISLAS',  -- NEW FIELD ADDED
...
$substation = 'ONCEISLAS'; // Only show reports for ONCEISLAS substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from ONCEISLAS substation
    $where = ["substation = 'ONCEISLAS'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from ONCEISLAS substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'ONCEISLAS' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for ONCEISLAS substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'ONCEISLAS' GROUP BY status";
...
<body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station ONCEISLAS</h3>
    </div>
    
    <nav>
        <ul>
            <li><a href="ONCEISLASview.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="ONCEISLASview.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="ONCEISLASview.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="ONCEISLASview.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="ONCEISLASview.php" method="get">
