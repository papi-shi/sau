...
function getTotalWorth($substation = 'BUUG') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'BUUG'));
...
function getMonthlyReportData($month, $year, $substation = 'BUUG') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'BUUG') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'BUUG') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
$substation = 'BUUG'; // Only show reports for BUUG substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from BUUG substation
    $where = ["substation = 'BUUG'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from BUUG substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'BUUG' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for BUUG substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'BUUG' GROUP BY status";
...
<!-- [Rest of your HTML remains the same] -->
 <body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station BUUG</h3>
    ...
    <nav>
        <ul>
            <li><a href="BUUG.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="BUUG.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="BUUG.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="BUUG.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="BUUG.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                ...
                <div>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="BUUG" selected>BUUG</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                       <label for="station">Station</label>
                        ...
                        <option value="Sibugay" selected>Sibugay</option>
                           </select>
                   </div>
...
<!-- JavaScript to Populate Description Based on Incident Type -->
    ...
    // Global variables
let map, marker;
const defaultLocation = [6.900923, 122.087026]; // Default Sibugay City coordinates
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="BUUG.php" method="get">
