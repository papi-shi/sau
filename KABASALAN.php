...
function getTotalWorth($substation = 'KABASALAN') {
    $conn = connectDB();
...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'KABASALAN'));
    ...
    $worth = isset($_POST['worth']) ? floatval(str_replace(',', '', $_POST['worth'])) : null;
...
function getMonthlyReportData($month, $year, $substation = 'KABASALAN') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'KABASALAN') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'KABASALAN') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'KABASALAN',  -- NEW FIELD ADDED
...
$substation = 'KABASALAN'; // Only show reports for KABASALAN substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from KABASALAN substation
    $where = ["substation = 'KABASALAN'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from KABASALAN substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'KABASALAN' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for KABASALAN substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'KABASALAN' GROUP BY status";
...
<body>
    <div class="header">
        ...
        <h3>Coast Guard Sub-Station KABASALAN</h3>
    ...
    <nav>
        <ul>
            <li><a href="KABASALAN.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="KABASALAN.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="KABASALAN.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="KABASALAN.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="KABASALAN.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="KABASALAN" selected>KABASALAN</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="KABASALAN.php" method="get">
