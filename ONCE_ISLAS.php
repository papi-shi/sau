...
function handleSubmission() {
    ...
    $substation = htmlspecialchars(trim($_POST['substation'] ?? 'ONCE ISLAS'));
...
function getMonthlyReportData($month, $year, $substation = 'ONCE ISLAS') {
    $conn = connectDB();
    $data = [
        'incidents' => [],
        'total' => 0,
...
function getQuarterlyReportData($quarter, $year, $substation = 'ONCE ISLAS') {
    $months = [];
    switch ($quarter) {
...
function getYearlyReportData($year, $substation = 'ONCE ISLAS') {
    $data = [
        'incidents' => [],
        'total' => 0,
        'by_type' => [],
...
function initializeDatabase() {
    ...
    $create_table = "CREATE TABLE IF NOT EXISTS `incidents` (
        ...
        `substation` VARCHAR(50) DEFAULT 'ONCE ISLAS',  -- NEW FIELD ADDED
...
$substation = 'ONCE ISLAS'; // Only show reports for ONCE ISLAS substation
...
if (!$show_report) {
    ...
    // Build query safely - only show incidents from ONCE ISLAS substation
    $where = ["substation = 'ONCE ISLAS'"];
    $params = [];
    $types = '';
    ...
    // Get incident types for filter dropdown (only from ONCE ISLAS substation)
    $incident_types = $conn->query("SELECT DISTINCT INCIDENT_TYPE FROM incidents WHERE substation = 'ONCE ISLAS' ORDER BY INCIDENT_TYPE");
    $conn->close();
}

// Get statistics for dashboard (only for ONCE ISLAS substation)
$counts = [
    'FOR FILLING' => 0,
    'FILED' => 0,
    'CLOSED' => 0
...
$conn = connectDB();
$sql = "SELECT status, COUNT(*) as count FROM incidents WHERE substation = 'ONCE ISLAS' GROUP BY status";
...
<body>
    ...
    <nav>
        <ul>
            <li><a href="ONCE ISLAS.php"><i class="fas fa-home"></i> Home</a></li>
            <li><a href="ONCE ISLAS.php" id="add-incident-btn"><i class="fas fa-plus-circle"></i> Add Incident</a></li>
            <li><a href="ONCE ISLAS.php" id="monthly-report-btn"><i class="fas fa-calendar-alt"></i> Monthly Report</a></li>
            <li><a href="ONCE ISLAS.php" id="quarterly-report-btn"><i class="fas fa-chart-bar"></i> Quarterly Report</a></li>
            <li><a href="ONCE ISLAS.php" id="yearly-report-btn"><i class="fas fa-chart-line"></i> Yearly Report</a></li>
        </ul>
    </nav>
                     ...
                     <div class="substation-highlight">
                        <div class="form-group">
                            ...
                            <select name="substation" id="substation" required>
                                <option value="ONCE ISLAS" selected>ONCE ISLAS</option>
                            </select>
                        </div>
                    </div>
...
function showReportSelectionModal(reportType) {
    ...
    const html = `
        ...
        <form action="ONCE ISLAS.php" method="get">
